...
| User invokes `/opsx:new` / `/opsx:ff` / `/opsx:propose` directly | Pass `--schema superpowers-bridge` explicitly (the repository default remains `spec-driven`); artifact instructions inject at each step |
