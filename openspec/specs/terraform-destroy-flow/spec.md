...
Defines the operator-facing `terraform destroy` flow: a self-bridged IPC channel that streams destroy output under the same durable apply lock as plan/apply, a fresh server-minted single-use confirmation token gate that prevents accidental destroys, a preload bridge exposing the destroy surface, and a type-to-confirm UI on the `/terraform` page.
