# guided-iam-provisioning Specification

## Purpose

Provides a one-click path for an operator with no existing AWS credentials to stand up a scoped deploy principal: a generated CloudFormation template creates the `HyveonDeployAll`-scoped IAM user and access key entirely in the operator's own AWS console session, the app intakes and validates the resulting bootstrap key, immediately rotates it via the OS keychain so the credential handed through the console never remains the one in long-term use, and requires the keychain to be available throughout so no credential is ever written to disk unencrypted.
