...
`IacController` SHALL expose an `iac.destroy` IPC channel following the shipped `iac.apply` streaming pattern: a self-bridged handler (registered via `onModuleInit` inside a real Electron main process, excluded from the generic bridge via `SELF_BRIDGED_PATTERNS` in `ipc-main-bridge.ts`) that resolves an immediate `{ started, runId?, error?, conflict? }` ack, streams every `PulumiService.destroy` chunk on a chunk side channel tagged with the run's id, sends exactly one terminal end message, and refuses submission with a `conflict` ack when the shared workspace is busy. `PulumiService.destroy`'s own self-contained gate MUST acquire the durable apply lock (`RunService.createRun`) before yielding its first chunk and release it on every exit path; the controller records an audit entry for accepted submissions and relies on `PulumiService.destroy`'s existing run-record persistence so destroy runs appear in run history.

#### Scenario: Destroy streams output and completes

- **WHEN** a valid destroy submission is accepted and the destroy run completes
...
- **THEN** the ack resolves `{ started: false, conflict }` naming the in-flight operation, no run is started, and no audit entry or run record is written

### Requirement: Fresh confirmation token gate

Every destroy attempt SHALL be gated on a fresh, server-minted, single-use, expiring confirmation token: the renderer requests a token via a plain-invoke IPC channel (`iac.destroy.mintToken`) backed by `PulumiService.mintDestroyConfirmationToken()`, and `iac.destroy` passes the supplied token to `PulumiService.destroy`, which refuses (per `DestroyNotConfirmedError`) when the token is absent, unknown, expired, or already consumed. The system MUST NOT ever run a destroy without this gate, and tokens MUST NOT be reusable across attempts.
...
- **THEN** the submission is refused with the `DestroyNotConfirmedError`-derived error, and no destroy run is started
