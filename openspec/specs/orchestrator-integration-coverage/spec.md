...
Defines the tier-2 integration coverage for the Terraform orchestrator (`TerraformService`/`TerraformController`) driven through the real DI container against a scripted fake `terraform` binary: PATH-based binary resolution, plan artifact/hash production, apply's pre-spawn approval and hash gates, destroy's confirmation-token gate, verbatim ANSI-preserving log streaming, run-record persistence, and the `output` subcommand.
