...
Defines the integration test coverage that exercises the infrastructure orchestrator (`PulumiService`) through the real DI container and controller wiring, using an in-process engine stub rather than a real Pulumi engine or a fake binary on `PATH`. It covers the preview/apply/destroy gates, ANSI-preserving streamed output, persisted run records, and stack-outputs handling that the desktop app depends on for correct plan/apply/destroy behavior.
