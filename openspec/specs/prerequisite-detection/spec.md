...
Defines detection of the local `terraform` and `aws` binaries the wizard depends on: a main-process service that probes PATH and parses tool versions, an IPC surface exposing the check to the renderer, and the first wizard step that blocks progression until both tools are present and shows OS-specific install guidance without ever attempting to install anything itself.
