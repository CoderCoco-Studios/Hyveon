...
- **THEN** the app SHALL render its own minimize, maximize/restore, and
  close buttons in the header, styled to match the app's theme, since the
  OS-native `titleBarOverlay` button set is dictated by the desktop
  environment's own window-manager layout (e.g. stock GNOME renders
  close-only) and cannot be relied on to include minimize/maximize
