...
Defines the TypeScript infrastructure program (`@hyveon/infra`) that replaces the HCL Terraform module: its packaging as an npm workspace, the typed game-server configuration model, configuration persisted as versioned JSON in the operator's S3 bucket, the exclusion of secret material from the stack, resource parity with the retired Terraform module, and the stack outputs contract the desktop app reads.
