...
- [ ] 2.2 Set `propagateTags: 'TASK_DEFINITION'` on the `RunTaskCommand` input in `app/packages/lambda/followup/src/handler.ts` (`runStart`) — this is a separate, independent `RunTask` call site used by the Discord `/start` command and is not covered by 2.1
...
- [ ] 3.4 Add/extend a unit test for the followup Lambda's `runStart` path asserting `propagateTags: 'TASK_DEFINITION'` is set on its `RunTaskCommand` input
