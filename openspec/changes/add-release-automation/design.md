...
### AI summary: `anthropics/claude-code-action`, authenticated with a subscription OAuth token, not a hand-rolled API script
Originally scoped as a hand-rolled Node script calling the Messages API directly with `@anthropic-ai/sdk`, on the reasoning that `anthropics/claude-code-action` is a full agentic harness (checkout, tool loop, PR/issue context) and is the wrong shape for a stateless one-shot summarization call. Revised after implementation: a direct API script only bills against a metered `ANTHROPIC_API_KEY`, while `claude_code_oauth_token` (minted via `claude setup-token`) is Anthropic's documented mechanism for a GitHub Actions workflow to bill against a Claude subscription instead of the metered API - see the [Claude Code GitHub Actions docs](https://code.claude.com/docs/en/github-actions). That billing difference outweighs the shape mismatch for this workflow, which runs rarely and has no other agentic-harness objection (the action isn't given git/bash tools, only `Read`/`Write`, so it behaves as a scoped file-in/file-out summarizer despite the heavier harness underneath). The action's `prompt` input reads git-cliff's rendered changelog and `--context` JSON from files already written to the checkout by the changelog-generation step and writes plain Markdown to `release-notes.md`, consumed by `softprops/action-gh-release`'s `body_path`. `claude_args` pins `--model claude-opus-5` and restricts `--allowedTools "Read,Write"` so the step cannot touch git state or push anything itself. The step runs with `continue-on-error: true`; a follow-up step falls back to the raw changelog if the action step failed or wrote no output.
...
2. `CLAUDE_CODE_OAUTH_TOKEN` secret is added (generated locally via `claude setup-token` under the Claude subscription the AI summary step should bill against).
3. This change's workflow and `cliff.toml` are merged to `main` via the normal PR process (this change itself is not exempt from review).
