...
- [ ] 1.1 Add `visibility?: 'public' | 'internal'` to `GameServerPort` in
...
- [ ] 1.2 Add the `visibility` enum to the port zod schema in
      `app/packages/shared/src/gameServerValidator.ts`.
- [ ] 1.3 Unit tests in `gameServerValidator.test.ts`: a port with
      `visibility` omitted validates; `'public'`/`'internal'` validate; any
      other string is rejected.

## 2. Security-group ingress

- [ ] 2.1 In `app/packages/infra/src/securityGroups.ts`, split
      ...
      key.
- [ ] 2.2 In `defineSecurityGroups`, resolve the VPC's CIDR block via
      `aws.ec2.getVpcOutput({ id: vpcId })`.
- [ ] 2.3 Build a second ingress-entry block for internal ports sourced
      ...
      `game_servers`.
- [ ] 2.4 Update TSDoc on `dedupedDirectGamePorts`/`SecurityGroupResources`
      to describe the two ingress sources.
- [ ] 2.5 Unit tests in `securityGroups.test.ts`: a game with one public
...
- [ ] 3.3 Show each port's visibility in
      ...
      lists ports.
- [ ] 3.4 Component/routed-page tests for the new control (jsdom project,
...
- [ ] 4.1 Update `docs/docs/components/infra.md`'s ingress-rule
      description to cover public vs. internal port ingress and the VPC
      CIDR source.
- [ ] 4.2 Note in the relevant `docs/docs/app/` wizard/games page(s) that a
      port can be marked VPC-only, and what that means for reachability.

## 5. Verification

- [ ] 5.1 `npm run app:lint` clean.
- [ ] 5.2 `npm run app:typecheck` clean.
- [ ] 5.3 `npm run app:test` full unit suite green.
- [ ] 5.4 `npm run app:test:integration` (Pulumi orchestration/security-group
      code changed).
