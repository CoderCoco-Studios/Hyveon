...
- [ ] 1.1 Add `archiver@8.0.0` (verify current version on the npm registry
...
- [ ] 2.1 Add an explicit safe-field allowlist mapping function for
...
- [ ] 2.2 Add a regex-based secret scrubber utility for log text (AWS
      access-key-id shape, generic long hex/base64 following a
      token/key/secret-ish label).
- [ ] 2.3 Unit tests: allowlist excludes non-listed fields (including a
...
- [ ] 3.1 Create `DiagnosticsBundleService` in `desktop-main`: gathers the
...
- [ ] 3.2 Map settled results to bundle content; failures become
      `errors.json` entries with `{ section, message }` only (no raw error
      objects), per `.claude/rules/logging.md`.
- [ ] 3.3 Stream the four sections plus `errors.json` into a `.zip` via
      `archiver`, written to a caller-supplied destination path.
- [ ] 3.4 Log entry/failure per `.claude/rules/logging.md` conventions
      (method name only on entry, warn/error on failure, no raw SDK errors
      escaping).
- [ ] 3.5 Unit tests: full success (all four sections present, empty
...
- [ ] 4.1 Add `diagnostics.exportBundle` `@MessagePattern` handler to
...
- [ ] 7.1 Update `docs/docs/components/management-app.md` with the new
      `diagnostics.exportBundle` IPC channel.
- [ ] 7.2 Update the relevant `docs/docs/app/` page (Settings/Diagnostics)
...
- [ ] 8.4 `npm run app:test:integration` green (IPC/controller changes).
- [ ] 8.5 `npm run app:test:e2e` green (renderer/preload/IPC surface
      changed).
