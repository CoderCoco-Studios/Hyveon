...
The reason recorded SHALL be truncated to a bounded length, so that a script cannot use it
to write unbounded operator-controlled content into the deployment's logs.
