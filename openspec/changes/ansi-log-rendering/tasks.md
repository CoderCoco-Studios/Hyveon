...
- [ ] 1.1 Create `app/packages/web/src/lib/ansi.utils.ts` containing
      `AnsiSegment`, `FG_COLOR_CLASS`, and `parseAnsiLine`, moved verbatim
      from `ansi-log-viewer.component.tsx`.
- [ ] 1.2 Extend the escape-sequence matching in `ansi.utils.ts` to
...
- [ ] 1.3 Add `stripAnsi(text: string): string` to `ansi.utils.ts`,
      removing all ANSI escape sequences from a string.
- [ ] 1.4 Update `ansi-log-viewer.component.tsx` to import
...
- [ ] 2.1 Update `log-line-display.component.tsx`'s `HighlightedLine` to
      ...
      unchanged.
- [ ] 2.2 Update `log-level.utils.ts`'s `detectLogLevel` to call
      `stripAnsi` on the input line before running its level-keyword
      regex.

## 3. Tests

- [ ] 3.1 Unit tests for `parseAnsiLine`/`stripAnsi` in
...
- [ ] 3.2 Component test for `HighlightedLine` asserting: (a) an
      ...
      current behavior.
- [ ] 3.3 Unit test for `detectLogLevel` confirming a level keyword
      wrapped in ANSI color codes still classifies correctly.
- [ ] 3.4 Run existing `ansi-log-viewer.component.spec` (Pulumi/IaC
