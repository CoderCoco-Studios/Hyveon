...
- [x] 1.1 Add `checkEnvironmentVariables(entry)` to
...
- [x] 1.2 Wire `checkEnvironmentVariables` into `validateGameServer`'s
...
- [x] 1.3 Add/extend `gameServerValidator.test.ts` (or create it if it
...
- [x] 2.1 Add `WizardDraftEnvironmentVariable { name: string; value: string }`
...
- [x] 2.2 Wire `environment` through `createEmptyWizardDraft` (empty array),
      ...
      `toProposedEntry`.
- [x] 2.3 Add `'environment'` to `WIZARD_STEPS` between `'storage'` and
      `'review'`, and extend `stepForIssuePath` so the `environment` path
      family routes to the new step.
- [x] 2.4 Add a `validateEnvironmentStep` export mirroring
      `validateStorageStep`.
- [x] 2.5 Extend `wizard-form.utils.test.ts` for the new draft field, step
...
- [x] 3.2 Add `environment-step.component.test.tsx`: row add/remove/edit,
      empty state message, validation issue display per row.

## 4. Wire the step into both flows

- [x] 4.1 In `add-game-wizard.component.tsx`, add `'Environment'` to
      `STEP_LABELS` and render `<EnvironmentStep>` between the Storage and
      Review steps.
- [x] 4.2 In `edit-game-form.component.tsx`, add an "Environment" `<Card>`
      section rendering `<EnvironmentStep>`, positioned after the Storage
      card.
- [x] 4.3 In `edit-game-form.component.tsx`, delete the
...
- [x] 4.4 Extend `add-game-wizard.component.test.tsx` and
...
- [x] 5.1 In `review-step.component.tsx`, add an "Environment variables"
      ...
      convention.
- [x] 5.2 Extend `review-step.component.test.tsx` for the new summary:
      rendered when rows exist, omitted when empty.

## 6. Documentation

- [x] 6.1 Update the add-game wizard / edit-game page(s) under
      `docs/docs/app/` to describe the new Environment step/section.
- [x] 6.2 Update `docs/docs/components/management-app.md` if it enumerates
      the wizard's steps.
- [x] 6.3 Run the `write-docs` skill (or its evaluator agents) over the
...
- [x] 7.3 `npm run app:test` — full unit suite green, including every new
      spec added above.
- [x] 7.4 `npm run app:test:e2e` — since the renderer changed (new wizard
