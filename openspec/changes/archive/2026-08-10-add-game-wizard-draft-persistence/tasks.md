...
- [x] 7.2 Run `npm run app:test:integration` — this change adds three `@MessagePattern` handlers to `GamesController` (`games.draft.get`/`save`/`clear`) backed by a new service, which the repo's pre-PR gate requires exercising through the in-process DI container, not just unit-level controller/service mocks.
- [x] 7.3 Run `npm run app:test:e2e` — the change adds renderer-side recovery behavior (the games-page resume/discard banner and its interaction with `AddGameWizard`'s `hideTrigger`/`onClose`) and a new preload/IPC surface (`games.draft.*`), both of which the repo's pre-PR gate requires covering when the renderer, preload bridge, or IPC surface changed. — 121/121 passed (54 chromium + 67 electron)
- [x] 7.4 Confirm the Docusaurus docs build cleanly (`npm run build` in `docs/`) after the `docs/docs/app/games.md` and `docs/docs/components/management-app.md` updates land. — build succeeded ("Generated static files in build")
