...
**Superseded 2026-07-26 — single PR, plus a scaffolder rework D5 didn't anticipate.** During `/opsx:apply`, the operator explicitly requested everything land as one PR — a straightforward scope reduction of D5, no functional change. Separately, verification surfaced that `scripts/init-parent.ts` (the private-parent-repo submodule scaffolder) generates a wrapper Makefile that fully depends on the root `setup.sh`/`Makefile` being deleted here (its `setup`/`update` targets shell out to `bash $(SUBMODULE)/setup.sh` and hash it for drift detection; `plan`/`apply`/`dev` delegate to `$(MAKE) -C $(SUBMODULE) tf-plan/tf-apply/dev`; it also writes `.env` with `API_TOKEN`). This was flagged only as a risk in the table below ("update the scaffolder/guide if so"), not resolved. Given the choice between deprecating the scaffolder, keeping the legacy root files, or a full rework, the operator chose **full rework**: `renderMakefile()` now inlines the `setup.sh` bootstrap logic directly (S3 state bucket + DynamoDB lock table + tfvars-S3-backend bootstrap via the untouched `terraform/bootstrap` module) and replaces the root-Makefile delegation with direct `terraform -chdir=$(TF_DIR)`/`npm run app:*` calls, with `.env`/`API_TOKEN` dropped entirely. See `tasks.md` §2.14 for the itemized breakdown and `docs/docs/guides/submodule.md` for the updated operator-facing description.
