...
- [x] 1.1 Add a `LambdaFunctionKey` union type (`'watchdog' | 'health-check' |
      'dns-updater' | 'interactions' | 'followup'`) to `@hyveon/shared`, exported for use
      by both `desktop-main` and `web`.

## 2. LogsService

- [x] 2.1 Add a private log-group resolver in `LogsService` that builds
      `/aws/lambda/${projectName}-${functionKey}`, reading `projectName` from
      `DeploymentConfigService` (default `'hyveon'`).
- [x] 2.2 Add `getRecentLambdaLogs(functionKey: LambdaFunctionKey, limit = 50):
      ...
      behavior.
- [x] 2.3 Add `streamLambdaLogs(functionKey: LambdaFunctionKey, signal: AbortSignal,
      ...
      design.md).
- [x] 2.4 Unit tests in `LogsService.test.ts` for 2.1-2.3: default/custom `projectName`
...
- [x] 3.1 Add `@MessagePattern('logs.lambda.get')` to `LogsController`, following
      `getRecentLogs`'s existing handler shape (`{ functionKey, limit? }` →
      `{ functionKey, lines }`), with a `logger.debug` entry line per
      `.claude/rules/logging.md`.
- [x] 3.2 Add a `logs.lambda.stream` self-bridged handler in `LogsController` (register
...
- [x] 3.3 Controller tests in `logs.controller.test.ts` for 3.1-3.2, mirroring the
      existing `logs.get`/`logs.stream` test coverage.
- [x] 3.4 Expose `logs.lambda.get` / `logs.lambda.stream` on the preload bridge
...
- [x] 4.1 In `app-layout.component.tsx`, replace the flat `Logs` `NavItem` with a `Logs`
...
- [x] 4.2 Preserve existing active-route highlighting semantics for both children
      (matches D3/D4 in design.md — only one child active at a time, based on exact
      path).
- [x] 4.3 Update/extend the existing sidebar component test(s) for the new nested
      structure and active-state behavior on `/logs` vs `/logs/infrastructure`.

## 5. Extract shared live-tail hook

- [x] 5.1 Extract a `useLogTail(target: string, api: LogTailApi)` hook out of
...
- [x] 5.2 Unit tests for the extracted hook in
      ...
      error sets `error`.
- [x] 5.3 Refactor `logs.page.tsx` to consume `useLogTail(selectedGame, window.hyveon.logs)`
...
- [x] 5.4 Verify `logs.page.test.tsx` passes unchanged (no test edits) as the regression
      gate for 5.3 — this file's existing assertions must all still hold against the
      refactored page.

## 6. Infrastructure logs page

- [x] 6.1 Add route `/logs/infrastructure` (router config wherever `/logs` is currently
      registered) rendering a new page component.
- [x] 6.2 Build the new page: a function picker (5 `LambdaFunctionKey` options) +
...
- [x] 6.3 On picker change, cancel the previous function's active stream before starting
      ...
      logic.
- [x] 6.4 Add a page object for the new page per this repo's Playwright conventions
      (`e2e/pages/`), exposing locators the same way `logs.page.tsx`'s existing page
      object does.
- [x] 6.5 jsdom routed-page spec for the new page, per
...
- [x] 7.1 chromium-project stub spec: nested sidebar renders both `Game Logs` and
      ...
      restarts the stream.
- [x] 7.2 Stub `logs.lambda.get`/`logs.lambda.stream` in the chromium bridge/fixtures
      alongside the existing `logs.get`/`logs.stream` stubs.

## 8. Docs

- [x] 8.1 Update `docs/docs/app/logs.md`: remove/correct the "It does not show Lambda
      logs" note; document the new `/logs/infrastructure` page, the function picker, and
      the nested sidebar structure.
- [x] 8.2 Update `docs/docs/components/management-app.md` (or wherever IPC
      channels/routes are enumerated) to list `logs.lambda.get` / `logs.lambda.stream`
      and the new route, if that page catalogs them.
- [x] 8.3 Update `docs/docs/components/integration-tests.md` if it names
...
- [x] 8.4 Run the `write-docs` skill's evaluator pass (accuracy, coverage, style) over
      the changed docs pages before opening the PR, per CLAUDE.md.

## 9. Pre-PR verification

- [x] 9.1 `npm run app:lint` clean.
- [x] 9.2 `npm run app:typecheck` clean.
- [x] 9.3 `npm run app:test` full unit suite green — including `use-log-tail.hook.test.ts`
      (new) and `logs.page.test.tsx` (unchanged, Task 5's regression gate).
- [x] 9.4 `npm run app:test:e2e` green (renderer/preload/IPC surface changed).
- [x] 9.5 `/opsx:sync` to fold `specs/infra-log-viewer/spec.md` into
