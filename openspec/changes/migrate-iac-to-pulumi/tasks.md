...
- [ ] 4.6 Report engine provisioning, provider plugin download, and the operation itself as distinct phases (engine-provisioning phase done — `PulumiEngineService.resolve(onPhase)`/`PulumiWorkspaceService.getOrCreateStack(onPhase)`; plugin-download and operation phases are still NOT fired anywhere — confirmed during Phase 10 bookkeeping that `PulumiService.ts` never threads `onPhase` through `getOrCreateStack` and never calls it around `stack.preview/up/destroy`. Genuinely still open, unlike 4.7-4.9 (which Phase 7 did close) — its remaining work is folded into task 10.3, the first concrete UI consumer of phase-reporting, rather than closed standalone — see task-4.5-4.6-report.md)
