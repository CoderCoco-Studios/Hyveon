...
- **Choice**: New pure function `estimateFargateHourlyCost(cpu: number, memory: number): number` in `@hyveon/shared`. The two pricing constants (`FARGATE_VCPU_PER_HOUR`, `FARGATE_GB_PER_HOUR`) move from `@hyveon/cloud-aws/AwsCloudProvider.ts` into `@hyveon/shared`; `cloud-aws` re-imports them so there is exactly one source of truth (same pattern as `canRun()` living in one place per `CLAUDE.md`). Rendered as `$X.XXXX/hr while running` (4 decimal places — needed so tiny estimates, e.g. the 0.25 vCPU / 0.5 GiB tier, still show a non-zero figure), recomputed synchronously on every slider `input` event.
