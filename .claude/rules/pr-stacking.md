...
2. One worktree + branch per group. First group branches from `main`; each later group branches from the *previous group's branch* — see `worktree.md` (fetch/update that base first). `EnterWorktree` can't target an arbitrary branch, and a direct `git worktree add` is blocked by the `guard-git-worktree-add-path` hook (worktree creation must go through `EnterWorktree`) — so for hop 2+, ask the user to run `git fetch origin <base-branch>` then `git worktree add -b <branch> .claude/worktrees/<name> origin/<base-branch>` themselves, then call `EnterWorktree` with `path` pointing at the resulting worktree to switch into it. Branching from the bare local `<base-branch>` skips the fetch and can silently drop commits pushed to that branch on GitHub.
