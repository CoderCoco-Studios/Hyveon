...
| `WindowController` | `window.minimize`, `window.toggleMaximize`, `window.close`, `window.isMaximized`, `window.maximizedChange` | Minimize, maximize, close, and query the maximized state of the main window. `window.maximizedChange` is a push event fired whenever the main window's maximized state changes, for any reason (button click, header double-click, OS-level snap/restore). |
