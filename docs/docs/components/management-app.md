...
| `WindowController` | `window.minimize`, `window.toggleMaximize`, `window.close`, `window.isMaximized`, `window.maximizedChange` | Minimize, maximize, close, and query the maximized state of the main window. `window.maximizedChange` is a push event fired whenever the main window's maximized state changes, for any reason (button click, header double-click, OS-level snap/restore). Backs the app header's merged title bar, rendered with platform-specific window-control buttons: native traffic lights on macOS, native titleBarOverlay on Windows, and app-drawn buttons on Linux. |
