...
- **The program is inline, not a file on disk.** Every stack operation goes
  through `PulumiWorkspaceService.getOrCreateStack()`, which builds the
  workspace itself via the lower-level `LocalWorkspace.create(opts)` (not the
  now-unused `createOrSelectStack()` convenience wrapper) and then calls
  `Stack.createOrSelect(PULUMI_STACK_NAME, ws)` — both from
  `@pulumi/pulumi/automation`. For `preview`
...
| `index.ts` | Barrel re-export of most `defineX()` functions, helpers, and types — but not all: `defineAll` (only called from `program.ts`), `stripTrailingDots`, `dedupedInternalGamePorts`, and `gamesWithHealthChecks` are internal to their own files and not re-exported here. | none |
