...
flowchart LR
  ...
  PMK  -- "npm install / npm run desktop:dev" --> UAPP
```
...
| `make dev` | Wipes stale TypeScript build info under the submodule's `app/packages/*/`, then runs `npm run desktop:dev` directly in the submodule. |
