...
/** Game name → declared health check, for the games opted into that verdict source instead of the CloudWatch heuristic. Populated by the infra program from `DeploymentConfig.gameServers`; empty when no game opts in. */
const HEALTH_CHECKS: Record<string, GameServerHealthCheck> = JSON.parse(process.env['HEALTH_CHECKS'] ?? '{}');
