 ...
 * slice of `TfvarsService.restoreRawTfvars` that `PulumiService.confirmRollback`
 * depends on) to the real `TfvarsService` singleton via `useExisting`, and
 * exports it — mirrors `run-record.module.ts`'s `RUN_RECORD_PERSISTER`/
 * `RUN_LOCK_SERVICE` bindings. This module is already imported directly by
 * `app.module.ts`, so `TFVARS_SERVICE` is reachable from anywhere in the
 * application container via `ModuleRef.get(TFVARS_SERVICE, { strict: false })`
 * without `pulumi-service.module.ts` ever needing a static `imports:` edge
 * back to this module — see `run-record.module.ts` for why that edge would
 * close a module cycle (`TfvarsModule` → `ConfigModule` →
