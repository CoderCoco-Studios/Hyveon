 ...
 * requirement.
 *
 * ## The exact message shape a DIY-backend lock conflict produces
 *
 * The self-managed (DIY/`file://`/`s3://`) backend's lock-conflict message is
 * generated by `checkForLock` in the Pulumi CLI's Go source
 * (`pkg/backend/diy/lock.go`, in the `pulumi/pulumi` GitHub repo — the Go
 * source is not vendored into this repo's `node_modules`, since only the
 * compiled CLI binary and the Node Automation API SDK are):
 *
 * ```go
 ...
 * module (`iac.controller.ts`'s pre-flight guard, which rejects a second
 * concurrent request using `PulumiService.getOperationInFlight()` as its
 * state accessor), *before* an operation ever reaches the SDK call this
 * module's classifier reacts to. What this module
