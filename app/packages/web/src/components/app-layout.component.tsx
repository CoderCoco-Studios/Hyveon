      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
        ...
        >
          ...
          <div className="flex items-center gap-4">
            ...
            {/*
              Windows' and Linux's titleBarOverlay reserves a region (top-right,
              per Electron's docs) where the OS draws the overlay buttons — DOM
              elements there cannot receive clicks. `env(titlebar-area-width)` is
              an absolute length (the draggable region's width, measured from the
              window's left edge), not a percentage — so it must be subtracted
              from the *viewport* width (`100vw`), not from `100%` of this
              spacer's own containing block (the trailing flex group, which is
              only a few hundred px wide and would make the formula always
              resolve negative → clamp to 0px, reserving nothing). `env(...)`
              only has a real value inside a WCO-enabled window (falls back to
              100vw elsewhere, so the whole expression evaluates to 0 outside
              win32/linux, which is fine since this element is gated to those
