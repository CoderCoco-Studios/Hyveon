...
describe('defineLambdas', () => {
  ...
  describe('CONNECT_MESSAGES / GAME_PORTS derivation', () => {
    ...
    it('should advertise the first public port in GAME_PORTS, not an internal-visibility port declared first', async () => {
      const gameServers: Record<string, GameServerConfig> = {
        managementFirst: {
          image: 'example/managed:latest',
          cpu: 1024,
          memory: 2048,
          ports: [
    ...
    it('should exclude a case-variant "ICMP" port from GAME_PORTS too', async () => {
      const gameServers: Record<string, GameServerConfig> = {
        echo: {
          image: 'example/echo:latest',
          cpu: 1024,
    ...
    it('should fall back to the first port when every declared port is internal-visibility', async () => {
      const gameServers: Record<string, GameServerConfig> = {
        allInternal: {
          image: 'example/internal-only:latest',
          cpu: 1024,
          memory: 2048,
          ports: [{ container: 9000, protocol: 'tcp', visibility: 'internal' }],
          volumes: [{ name: 'saves', container_path: '/data' }],
        },
      };
      const { provider, roles, efs } = await arrangeDependencies(gameServers);
      ...
      const fn = findByName(mocks.resources, 'hyveon-followup');
      const variables = (fn.inputs.environment as { variables: Record<string, unknown> }).variables;
      expect(JSON.parse(variables.GAME_PORTS as string)).toEqual({ allInternal: 9000 });
    });
  });
